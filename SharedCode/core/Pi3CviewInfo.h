...
public:
	...
	vec2f getMouseToWorldCoords(const int mx, const int my) {
		vec2f cpos((float)(-(mx - viewport.x - viewport.width / 2)), (float)(-(my - viewport.y - viewport.height / 2)));
		float iz = (projection == ORTHOGRAPHIC) ? 2.f / zoom : zoom / pspvalue;
		return cpos * iz; // +pan;
	}
