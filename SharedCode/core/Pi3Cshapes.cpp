...
namespace Pi3Cshapes {
	...
	void extrude_verts(std::vector<float> &verts, uint32_t &vc, const vec3f &pos, std::vector<std::vector<vec2f>>& contoursxy, const float depth, const uint16_t divs, const uint32_t col) {
	...
	Pi3Cmesh extrude(const std::string &name, const vec3f &pos, std::vector<std::vector<vec2f>> &contoursxy, const float depth, const uint32_t col, const uint16_t divs)
	{
		Pi3Cmesh extrusion(name);
		extrusion.stride = 9;

		extrude_verts(extrusion.verts, extrusion.vc, pos, contoursxy, depth, divs, col);
