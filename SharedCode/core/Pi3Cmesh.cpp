...
void Pi3Cmesh::insideRect(Pi3Cbbox2d& bbox, std::vector<uint32_t>& pointIndexes, std::vector<uint32_t>& edgeIndexes, std::vector<uint32_t>& faceIndexes, 
	const float perspective, const Pi3Cmatrix& mtx, const std::vector<float>& mverts)
{
	uint32_t vsize = vertSize * stride;
	uint32_t voff = vertOffset * stride;
	...
	for (size_t i = voff; i < (voff + vsize); i += stride * 3)
	{
		...
		vec3f v1 = mtx.transformVec(&mverts[i]);
		vec3f v2 = mtx.transformVec(&mverts[i2]);
		vec3f v3 = mtx.transformVec(&mverts[i3]);
